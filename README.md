...
Alerts are tiered as `GOLD`, `SILVER`, or `WATCHLIST`. Telegram sends `GOLD` by default, sends `SILVER` only when enabled, and logs `WATCHLIST` candidates without sending them. Market ranking is tuned for likely winners first: Hits+Runs+RBIs and Hits are the normal priority, Total Bases needs a stronger score, and RBI/Home Run are reserved for obvious matchup gems. Home Run markets can still lead an alert, but only when the score, power profile, elite hitter profile, and pitcher weakness all support it.
...
Settled sportsbook bets can be logged without an alert ID using `/trainbet`, for example `/trainbet win +340 1 Player Hits | Sam Antonacci 2+ hits`. These records are stored in `bet_training.json` and shown in `/training`. Screenshot history can also be manually converted into `bet_training.json` when typing every result would be too slow. Screenshot bets are marked with `matched_alert_id` when they correspond to a bot alert; unmatched sportsbook bets are kept for context but do not train alert scoring by default.
